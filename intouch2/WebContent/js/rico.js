...
Rico.Accordion.prototype = {
   ...
   setOptions: function(options) {
	  var mainH = document.documentElement.clientHeight - 82;
	  if (window.opera) mainH = document.body.clientHeight - 82;
	  if (navigator.appVersion.indexOf("Safari") != -1) mainH = self.innerHeight - 82;
	  if (mainH < 400) mainH = 200;
...
Rico.DragAndDrop.prototype = {
   ...
   _mouseMoveHandler: function(e) {
      ...
      {
	      if(document.all && e.x == this.startx && e.y == this.starty) return;
		  var ctrl = false;
		  try {ctrl = (e.ctrlKey) ? true : false;} catch (jj) {}
   		  if(ctrl) return;
		  this._startDrag(e);
	  }
   ...
   **/
   ...
   _updateDraggableLocation: function(e) {
      var dragObjectStyle = this.dragElement.style;
		if(document.all)
		{
			dragObjectStyle.left = event.x + 10 + "px";
			dragObjectStyle.top = event.y + 10 + "px";
		}
		else
		{
   ...
   _toAbsolute: function(element,accountForDocScroll) {
      ...
      while ( parent ) {
         ...
         if ( parent != element ) {
            ...
            var bl = this.getElementsComputedStyle(parent, "borderLeftWidth" );
            if (bl == 'medium') {
            	bl = "2";
            }
            var bt = this.getElementsComputedStyle(parent, "borderTopWidth" );
			if (bt == 'medium') {
				bt = "2";
			}

            var borderXOffset = parseInt(bl);
            var borderYOffset = parseInt(bt);
...
//				 alert("offsetParent : " + parent.offsetParent + " id: " + parent.id);
